...
setInterval(() => {
    ...
    // Converte relógio 24h em 12h
    // if(h > 12){
    //     h = h - 12
    // }

    // Adiciona zero antes do dígito único
    h = (h < 10) ? "0" + h : h
    m = (m < 10) ? "0" + m : m
    s = (s < 10) ? "0" + s : s

    hours.innerHTML = h + "<br><span>hora(s)</span>"
    minutes.innerHTML = m + "<br><span>minutos(s)</span>"
    seconds.innerHTML = s + "<br><span>segundo(s)</span>"
